...
public class ShapeTouch : AbstractShape
{
    public override void StartShapeScript(SpellScript SS)
    {
        Debug.Log("Touch shape script started");

        damageModifier = 1f; speedModifier = 1f; radiusModifier = 1f; cooldownModifier = 1f;
        shapeMesh = Resources.Load<Mesh>("CustomMeshes/shapeTouch");
    ...
    {
        Debug.Log("Touch shape aim spell");
    }

    public override void UpdateAimPath(Vector3[] addPoints)
    {
        Debug.Log("Touch shape update aim path");
    }
